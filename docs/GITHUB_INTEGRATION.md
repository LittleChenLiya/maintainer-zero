...
For a copy-paste consumer workflow, start with the examples/consumer-workflow.yml file. It runs on pushes to any branch, pull requests, a monthly schedule, or manual dispatch; it intentionally does not assume the adopting repository's default branch is named `main`. It uses the immutable commit for the v0.2.2 alpha preview, keeps contents: read, validates the report/manifest/credential with the Action code path, and uploads only verified report artifacts. Review a newer release before changing the pinned ref, then follow the [first-run checklist](CONSUMER_WORKFLOW_CHECKLIST.md).

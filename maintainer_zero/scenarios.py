...
def dependency_yanked(repo: RepoSnapshot, days: int = 30) -> DrillResult:
    ...
    if count:
        findings = [Finding("high" if count >= 5 else "medium", "依赖撤包冲击", f"模拟关键依赖不可用：{', '.join(critical)}。", "锁定来源和版本，准备替代包，并定期验证冷构建。")]
        applicable = True
        events = [(0, "上游包被撤下", "新环境无法完成安装"), (1, "锁文件重建失败", "发布和安全修复流水线受阻"), (days, "演练结束", "验证缓存、镜像和替代依赖")]
    else:
        findings = [Finding("info", "没有可演练的清单依赖", "未发现 package.json 或 requirements 文件中的依赖；撤包场景不适用。", "确认项目是否通过其他清单或构建系统声明依赖。")]
        applicable = False

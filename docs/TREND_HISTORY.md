...
`simulate --history PATH` 会把当前报告追加到一个本地 JSON 历史文件，并在输出目录生成 `history-summary.json` 和不含本地路径的 `history-summary.md`。历史只保存分数、场景分数、Finding 数量和高危 Finding 数量，不复制仓库内容、原始事件或贡献者记录。
历史文件通过同目录临时文件和原子替换写入；如果进程在写入期间失败，旧历史不会被截断。
...
Markdown 摘要只显示仓库名称、规则版本和聚合变化，不显示本地路径或原始报告；未知值显示为 `unknown`，不会被渲染为零。

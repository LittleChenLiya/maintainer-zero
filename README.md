...
A copy-paste consumer workflow is available at examples/consumer-workflow.yml. It runs on main pushes, pull requests, a monthly schedule, or manual dispatch; pins the reviewed v0.2.2 alpha commit, runs on Ubuntu and Windows, exports the Action code path for follow-up validation, and verifies report integrity before upload.
